# Push experiments: setup and metrics

Generated by `tools/scorecard.py` on every evaluation; do not edit by hand.
Metrics on `e75`/`e23` rows are computed on the frozen **23-object** list; other sets as
recorded. `harness`: rows tagged *corrected harness* were evaluated after the 2026-08-29
settle fix; earlier rows had goal clouds floating up to ~9 cm and are precision-depressed.

## Setup

| run | eval | variant | stack | backbone | actions | cross-attn | aux head | goal cond | cloud norm | data | eps | steps |
|---|---|---|---|---|---|---|---|---|---|---|---|---|
| `push_act_objabs` | e23 | EMA, corrected harness | new | unet | absolute | no | none | ? | workspace | push_objonly | all | 100000 |
| `push_act_xattn` | e23 | EMA, corrected harness | new | unet | absolute | yes | none | ? | workspace | push_objonly | all | 100000 |
| `push_dit_objabs` | e23 | EMA | new | dit | absolute | no | none | both | workspace | push_objonly | all | 100000 |
| `push_dit_objabs` | e23 | EMA, corrected harness | new | dit | absolute | no | none | both | workspace | push_objonly | all | 100000 |
| `push_dit_objabs_nogoalvec` | e23 | EMA, corrected harness | new | dit | absolute | no | none | points | workspace | push_objonly | all | 100000 |
| `push_dit_objabs_xattn` | e23 | EMA, corrected harness | new | dit | absolute | yes | none | both | workspace | push_objonly | all | 100000 |
| `push_dit_objabs_xattn_novec` | e23 | EMA, corrected harness | new | dit | absolute | yes | none | points | workspace | push_objonly | all | 100000 |
| `push_dit_objdelta` | e23 | EMA | new | dit | delta | no | none | both | workspace | push_objonly_delta | all | 100000 |
| `push_dp3_objabs` | e23 | EMA, corrected harness | new | unet | absolute | no | none | points | workspace | push_objonly | all | 100000 |
| `push_v2_act` | seenv2 | EMA, corrected harness | old | unet | absolute | no | none | ? | workspace | push_v2obj | 1545 | 100000 |
| `push_v2_act` | unseenv2 | EMA, corrected harness | old | unet | absolute | no | none | ? | workspace | push_v2obj | 1545 | 100000 |
| `push_v2_base` | seenv2 | EMA, corrected harness | old | dit | absolute | no | none | both | workspace | push_v2obj | 1545 | 100000 |
| `push_v2_base` | unseenv2 | EMA, corrected harness | old | dit | absolute | no | none | both | workspace | push_v2obj | 1545 | 100000 |
| `push_v2_dp3` | seenv2 | EMA, corrected harness | old | unet | absolute | no | none | points | workspace | push_v2obj | 1545 | 100000 |
| `push_v2_dp3` | unseenv2 | EMA, corrected harness | old | unet | absolute | no | none | points | workspace | push_v2obj | 1545 | 100000 |
| `push_v2_neither` | seenv2 | EMA, corrected harness | old | dit | absolute | no | none | points | workspace | push_v2obj | 1545 | 100000 |
| `push_v2_neither` | unseenv2 | EMA, corrected harness | old | dit | absolute | no | none | points | workspace | push_v2obj | 1545 | 100000 |
| `push_v2_xattn` | seenv2 | EMA, corrected harness | old | dit | absolute | yes | none | both | workspace | push_v2obj | 1545 | 100000 |
| `push_v2_xattn` | unseenv2 | EMA, corrected harness | old | dit | absolute | yes | none | both | workspace | push_v2obj | 1545 | 100000 |
| `push_v2_xattn_novec` | seenv2 | EMA, corrected harness | old | dit | absolute | yes | none | points | workspace | push_v2obj | 1545 | 100000 |
| `push_v2_xattn_novec` | unseenv2 | EMA, corrected harness | old | dit | absolute | yes | none | points | workspace | push_v2obj | 1545 | 100000 |
| `push_v3all_act` | x75a_s1234 | EMA, corrected harness | old | unet | absolute | no | none | ? | workspace | push_v3obj | all | 100000 |
| `push_v3all_act` | x75a_s2345 | EMA, corrected harness | old | unet | absolute | no | none | ? | workspace | push_v3obj | all | 100000 |
| `push_v3all_act` | x75a_s3456 | EMA, corrected harness | old | unet | absolute | no | none | ? | workspace | push_v3obj | all | 100000 |
| `push_v3all_act` | x75b_s1234 | EMA, corrected harness | old | unet | absolute | no | none | ? | workspace | push_v3obj | all | 100000 |
| `push_v3all_act` | x75b_s2345 | EMA, corrected harness | old | unet | absolute | no | none | ? | workspace | push_v3obj | all | 100000 |
| `push_v3all_act` | x75b_s3456 | EMA, corrected harness | old | unet | absolute | no | none | ? | workspace | push_v3obj | all | 100000 |
| `push_v3all_base` | x75a_s1234 | EMA, corrected harness | old | dit | absolute | no | none | both | workspace | push_v3obj | all | 100000 |
| `push_v3all_base` | x75a_s2345 | EMA, corrected harness | old | dit | absolute | no | none | both | workspace | push_v3obj | all | 100000 |
| `push_v3all_base` | x75a_s3456 | EMA, corrected harness | old | dit | absolute | no | none | both | workspace | push_v3obj | all | 100000 |
| `push_v3all_base` | x75b_s1234 | EMA, corrected harness | old | dit | absolute | no | none | both | workspace | push_v3obj | all | 100000 |
| `push_v3all_base` | x75b_s2345 | EMA, corrected harness | old | dit | absolute | no | none | both | workspace | push_v3obj | all | 100000 |
| `push_v3all_base` | x75b_s3456 | EMA, corrected harness | old | dit | absolute | no | none | both | workspace | push_v3obj | all | 100000 |
| `push_v3all_dp3` | x75a_s1234 | EMA, corrected harness | old | unet | absolute | no | none | points | workspace | push_v3obj | all | 100000 |
| `push_v3all_dp3` | x75a_s2345 | EMA, corrected harness | old | unet | absolute | no | none | points | workspace | push_v3obj | all | 100000 |
| `push_v3all_dp3` | x75a_s3456 | EMA, corrected harness | old | unet | absolute | no | none | points | workspace | push_v3obj | all | 100000 |
| `push_v3all_dp3` | x75b_s1234 | EMA, corrected harness | old | unet | absolute | no | none | points | workspace | push_v3obj | all | 100000 |
| `push_v3all_dp3` | x75b_s2345 | EMA, corrected harness | old | unet | absolute | no | none | points | workspace | push_v3obj | all | 100000 |
| `push_v3all_dp3` | x75b_s3456 | EMA, corrected harness | old | unet | absolute | no | none | points | workspace | push_v3obj | all | 100000 |
| `push_v3all_dp3_267m` | x75a_s1234 | EMA, corrected harness | old | unet | absolute | no | none | points | workspace | push_v3obj | all | 100000 |
| `push_v3all_dp3_267m` | x75a_s2345 | EMA, corrected harness | old | unet | absolute | no | none | points | workspace | push_v3obj | all | 100000 |
| `push_v3all_dp3_267m` | x75a_s3456 | EMA, corrected harness | old | unet | absolute | no | none | points | workspace | push_v3obj | all | 100000 |
| `push_v3all_dp3_267m` | x75b_s1234 | EMA, corrected harness | old | unet | absolute | no | none | points | workspace | push_v3obj | all | 100000 |
| `push_v3all_dp3_267m` | x75b_s2345 | EMA, corrected harness | old | unet | absolute | no | none | points | workspace | push_v3obj | all | 100000 |
| `push_v3all_dp3_267m` | x75b_s3456 | EMA, corrected harness | old | unet | absolute | no | none | points | workspace | push_v3obj | all | 100000 |
| `push_v3all_neither` | x75a_s1234 | EMA, corrected harness | old | dit | absolute | no | none | points | workspace | push_v3obj | all | 100000 |
| `push_v3all_neither` | x75a_s2345 | EMA, corrected harness | old | dit | absolute | no | none | points | workspace | push_v3obj | all | 100000 |
| `push_v3all_neither` | x75a_s3456 | EMA, corrected harness | old | dit | absolute | no | none | points | workspace | push_v3obj | all | 100000 |
| `push_v3all_neither` | x75b_s1234 | EMA, corrected harness | old | dit | absolute | no | none | points | workspace | push_v3obj | all | 100000 |
| `push_v3all_neither` | x75b_s2345 | EMA, corrected harness | old | dit | absolute | no | none | points | workspace | push_v3obj | all | 100000 |
| `push_v3all_neither` | x75b_s3456 | EMA, corrected harness | old | dit | absolute | no | none | points | workspace | push_v3obj | all | 100000 |
| `push_v3all_xattn` | x75a_s1234 | EMA, corrected harness | old | dit | absolute | yes | none | both | workspace | push_v3obj | all | 100000 |
| `push_v3all_xattn` | x75a_s2345 | EMA, corrected harness | old | dit | absolute | yes | none | both | workspace | push_v3obj | all | 100000 |
| `push_v3all_xattn` | x75a_s3456 | EMA, corrected harness | old | dit | absolute | yes | none | both | workspace | push_v3obj | all | 100000 |
| `push_v3all_xattn` | x75b_s1234 | EMA, corrected harness | old | dit | absolute | yes | none | both | workspace | push_v3obj | all | 100000 |
| `push_v3all_xattn` | x75b_s2345 | EMA, corrected harness | old | dit | absolute | yes | none | both | workspace | push_v3obj | all | 100000 |
| `push_v3all_xattn` | x75b_s3456 | EMA, corrected harness | old | dit | absolute | yes | none | both | workspace | push_v3obj | all | 100000 |
| `push_v3all_xattn_novec` | x75a_s1234 | EMA, corrected harness | old | dit | absolute | yes | none | points | workspace | push_v3obj | all | 100000 |
| `push_v3all_xattn_novec` | x75a_s2345 | EMA, corrected harness | old | dit | absolute | yes | none | points | workspace | push_v3obj | all | 100000 |
| `push_v3all_xattn_novec` | x75a_s3456 | EMA, corrected harness | old | dit | absolute | yes | none | points | workspace | push_v3obj | all | 100000 |
| `push_v3all_xattn_novec` | x75b_s1234 | EMA, corrected harness | old | dit | absolute | yes | none | points | workspace | push_v3obj | all | 100000 |
| `push_v3all_xattn_novec` | x75b_s2345 | EMA, corrected harness | old | dit | absolute | yes | none | points | workspace | push_v3obj | all | 100000 |
| `push_v3all_xattn_novec` | x75b_s3456 | EMA, corrected harness | old | dit | absolute | yes | none | points | workspace | push_v3obj | all | 100000 |

## Metrics

| run | eval | variant | n | **primary 50mm/0.2** | 95% CI | strict 30mm/0.15 | 40mm/0.15 | pos gate | ori gate | ever-in-goal | pos mean/med/std/p90 (mm) | ori mean/med/std/p90 (deg) |
|---|---|---|---|---|---|---|---|---|---|---|---|---|
| `push_v3all_xattn` | x75b_s2345 | EMA, corrected harness | 264 | **87.9%** | 83.4–91.3 | 76.5% | 79.5% | 93.6% | 89.4% | 89.8% | 18.3 / 12.3 / 20.4 / 39.5 | 7.1 / 3.5 / 13.1 / 11.7 |
| `push_v3all_xattn` | x75b_s1234 | EMA, corrected harness | 264 | **86.0%** | 81.3–89.7 | 73.1% | 76.1% | 93.6% | 87.9% | 89.8% | 18.5 / 11.8 / 19.6 / 39.0 | 7.1 / 3.6 / 13.0 / 12.8 |
| `push_v3all_base` | x75b_s3456 | EMA, corrected harness | 264 | **84.5%** | 79.6–88.3 | 69.3% | 75.4% | 93.6% | 86.7% | 90.2% | 22.2 / 16.2 / 22.2 / 41.7 | 6.4 / 3.8 / 10.0 / 13.0 |
| `push_v3all_xattn` | x75b_s3456 | EMA, corrected harness | 264 | **84.5%** | 79.6–88.3 | 75.4% | 78.8% | 93.6% | 86.0% | 86.4% | 20.2 / 13.3 / 26.8 / 40.1 | 8.7 / 3.4 / 17.6 / 16.7 |
| `push_v3all_xattn_novec` | x75b_s1234 | EMA, corrected harness | 264 | **83.7%** | 78.8–87.7 | 71.2% | 75.4% | 93.6% | 85.2% | 87.1% | 19.9 / 11.8 / 34.7 / 37.5 | 8.4 / 3.8 / 16.7 / 16.1 |
| `push_v3all_xattn_novec` | x75b_s2345 | EMA, corrected harness | 264 | **83.7%** | 78.8–87.7 | 73.9% | 76.1% | 94.3% | 85.6% | 86.7% | 17.3 / 11.4 / 20.5 / 33.4 | 7.8 / 4.4 / 13.6 / 15.5 |
| `push_dit_objabs_xattn` | e23 | EMA, corrected harness | 276 | **83.3%** | 78.5–87.3 | 72.8% | 74.6% | 86.6% | 88.4% | 89.9% | 25.0 / 14.8 / 30.5 / 67.3 | 6.7 / 3.2 / 10.6 / 14.2 |
| `push_v3all_xattn` | x75a_s1234 | EMA, corrected harness | 276 | **83.3%** | 78.5–87.3 | 71.4% | 74.6% | 92.0% | 86.2% | 89.1% | 20.9 / 14.0 / 21.8 / 44.7 | 6.9 / 3.7 / 11.7 / 13.9 |
| `push_dit_objabs_xattn_novec` | e23 | EMA, corrected harness | 276 | **82.2%** | 77.3–86.3 | 72.5% | 77.9% | 87.3% | 85.5% | 90.6% | 24.3 / 14.7 / 31.5 / 61.6 | 6.9 / 3.2 / 10.8 / 19.3 |
| `push_v3all_xattn` | x75a_s2345 | EMA, corrected harness | 276 | **81.9%** | 76.9–86.0 | 71.7% | 75.7% | 92.8% | 84.1% | 86.6% | 19.9 / 13.2 / 18.8 / 43.7 | 8.4 / 4.2 / 14.8 / 17.8 |
| `push_v3all_xattn_novec` | x75b_s3456 | EMA, corrected harness | 264 | **81.8%** | 76.7–86.0 | 72.0% | 75.0% | 94.3% | 83.7% | 86.4% | 16.9 / 11.8 / 17.0 / 35.6 | 7.0 / 3.8 / 9.0 / 20.1 |
| `push_v3all_base` | x75a_s2345 | EMA, corrected harness | 276 | **81.5%** | 76.5–85.7 | 69.2% | 73.6% | 91.7% | 84.1% | 85.5% | 22.7 / 16.7 / 20.8 / 45.8 | 7.7 / 4.2 / 12.9 / 16.3 |
| `push_v3all_neither` | x75a_s2345 | EMA, corrected harness | 276 | **81.5%** | 76.5–85.7 | 67.0% | 70.7% | 94.9% | 83.0% | 88.0% | 21.5 / 15.8 / 25.6 / 38.0 | 7.3 / 3.9 / 10.0 / 18.4 |
| `push_v3all_xattn` | x75a_s3456 | EMA, corrected harness | 276 | **81.5%** | 76.5–85.7 | 71.4% | 73.2% | 92.4% | 84.8% | 87.3% | 19.3 / 12.9 / 19.7 / 43.5 | 7.3 / 3.9 / 11.7 / 15.2 |
| `push_v3all_base` | x75a_s3456 | EMA, corrected harness | 276 | **81.2%** | 76.1–85.3 | 63.8% | 68.5% | 92.8% | 84.4% | 88.4% | 22.4 / 16.7 / 22.7 / 43.7 | 7.6 / 4.5 / 13.1 / 15.0 |
| `push_v3all_neither` | x75a_s1234 | EMA, corrected harness | 276 | **81.2%** | 76.1–85.3 | 70.7% | 75.7% | 94.6% | 82.6% | 87.0% | 19.2 / 13.8 / 18.3 / 34.7 | 6.4 / 3.4 / 8.7 / 15.7 |
| `push_v3all_neither` | x75a_s3456 | EMA, corrected harness | 276 | **81.2%** | 76.1–85.3 | 66.3% | 72.5% | 94.9% | 83.0% | 87.7% | 19.6 / 14.4 / 17.9 / 38.1 | 7.0 / 4.3 / 9.2 / 15.5 |
| `push_v3all_neither` | x75b_s3456 | EMA, corrected harness | 264 | **81.1%** | 75.9–85.3 | 66.7% | 70.8% | 94.3% | 82.6% | 87.1% | 19.6 / 15.1 / 19.4 / 39.8 | 7.8 / 4.1 / 12.1 / 17.5 |
| `push_dit_objabs_nogoalvec` | e23 | EMA, corrected harness | 276 | **80.8%** | 75.7–85.0 | 68.8% | 75.7% | 90.6% | 84.8% | 86.2% | 24.2 / 16.6 / 25.9 / 49.7 | 7.3 / 3.3 / 11.7 / 18.6 |
| `push_v3all_xattn_novec` | x75a_s2345 | EMA, corrected harness | 276 | **80.8%** | 75.7–85.0 | 69.9% | 72.5% | 93.1% | 84.4% | 85.9% | 83.6 / 11.7 / 1072.9 / 38.2 | 8.5 / 4.3 / 16.6 / 14.5 |
| `push_v3all_xattn_novec` | x75a_s1234 | EMA, corrected harness | 276 | **80.4%** | 75.4–84.7 | 69.9% | 73.6% | 91.3% | 83.7% | 87.0% | 20.5 / 13.0 / 26.8 / 40.5 | 8.1 / 4.4 / 14.2 / 14.7 |
| `push_v3all_base` | x75b_s2345 | EMA, corrected harness | 264 | **79.9%** | 74.7–84.3 | 66.3% | 72.0% | 91.7% | 81.8% | 85.2% | 23.2 / 16.2 / 24.7 / 47.6 | 7.4 / 4.0 / 10.8 / 16.3 |
| `push_dit_objabs` | e23 | EMA, corrected harness | 276 | **79.3%** | 74.2–83.7 | 67.4% | 72.8% | 86.6% | 84.4% | 87.0% | 27.7 / 14.7 / 36.5 / 71.0 | 7.6 / 3.5 / 12.2 / 19.4 |
| `push_v3all_xattn_novec` | x75a_s3456 | EMA, corrected harness | 276 | **79.3%** | 74.2–83.7 | 69.2% | 71.4% | 94.2% | 80.8% | 84.8% | 19.7 / 12.1 / 27.9 / 40.9 | 9.3 / 4.5 / 15.7 / 20.3 |
| `push_v3all_base` | x75b_s1234 | EMA, corrected harness | 264 | **79.2%** | 73.9–83.6 | 64.8% | 69.3% | 88.3% | 83.7% | 84.5% | 24.1 / 16.3 / 25.3 / 53.0 | 8.5 / 4.1 / 14.7 / 20.5 |
| `push_v3all_neither` | x75b_s1234 | EMA, corrected harness | 264 | **79.2%** | 73.9–83.6 | 64.0% | 67.8% | 94.7% | 79.5% | 84.1% | 19.1 / 13.4 / 21.4 / 37.7 | 8.6 / 4.1 / 13.7 / 19.8 |
| `push_v3all_base` | x75a_s1234 | EMA, corrected harness | 276 | **79.0%** | 73.8–83.4 | 64.1% | 72.1% | 89.9% | 83.0% | 86.6% | 23.5 / 17.2 / 22.2 / 50.5 | 7.0 / 4.2 / 8.9 / 15.7 |
| `push_v3all_neither` | x75b_s2345 | EMA, corrected harness | 264 | **77.7%** | 72.3–82.3 | 67.4% | 68.6% | 93.9% | 79.5% | 84.8% | 19.6 / 13.4 / 23.1 / 42.6 | 8.1 / 4.0 / 11.8 / 17.6 |
| `push_v3all_act` | x75a_s1234 | EMA, corrected harness | 276 | **77.2%** | 71.9–81.7 | 58.7% | 64.1% | 88.4% | 80.4% | 80.8% | 25.7 / 18.8 / 24.0 / 57.1 | 8.3 / 5.2 / 11.7 / 15.7 |
| `push_dit_objabs` | e23 | EMA | 276 | **76.8%** | 71.5–81.4 | 62.0% | 68.8% | 84.8% | 82.2% | 85.1% | 27.8 / 18.5 / 28.7 / 61.1 | 8.1 / 4.3 / 10.5 / 19.7 |
| `push_v3all_act` | x75a_s3456 | EMA, corrected harness | 276 | **76.1%** | 70.7–80.7 | 55.1% | 63.8% | 88.0% | 80.4% | 84.1% | 28.7 / 18.2 / 33.6 / 53.5 | 8.7 / 5.5 / 12.1 / 16.5 |
| `push_v3all_act` | x75b_s3456 | EMA, corrected harness | 264 | **75.8%** | 70.2–80.5 | 56.4% | 62.5% | 89.0% | 79.9% | 83.7% | 30.1 / 18.9 / 41.9 / 53.7 | 8.5 / 5.3 / 12.0 / 17.9 |
| `push_v3all_act` | x75a_s2345 | EMA, corrected harness | 276 | **73.9%** | 68.4–78.7 | 60.9% | 64.5% | 88.0% | 77.5% | 81.2% | 26.6 / 17.4 / 34.1 / 59.0 | 8.5 / 5.7 / 9.9 / 18.9 |
| `push_v3all_act` | x75b_s1234 | EMA, corrected harness | 264 | **73.1%** | 67.5–78.1 | 59.1% | 63.6% | 87.9% | 78.4% | 79.5% | 29.5 / 18.6 / 38.8 / 57.2 | 8.6 / 5.4 / 12.2 / 17.9 |
| `push_v2_neither` | unseenv2 | EMA, corrected harness | 240 | **72.9%** | 67.0–78.1 | 59.6% | 63.3% | 88.3% | 76.7% | 84.2% | 24.3 / 17.0 / 23.3 / 59.2 | 8.0 / 4.9 / 9.8 / 18.9 |
| `push_v2_xattn` | unseenv2 | EMA, corrected harness | 240 | **72.9%** | 67.0–78.1 | 63.3% | 67.5% | 87.5% | 75.0% | 84.6% | 25.3 / 15.1 / 29.2 / 58.5 | 9.6 / 5.0 / 15.2 / 22.4 |
| `push_v2_xattn` | seenv2 | EMA, corrected harness | 276 | **72.5%** | 66.9–77.4 | 59.8% | 63.8% | 89.9% | 74.6% | 76.8% | 23.8 / 16.7 / 25.2 / 50.1 | 10.4 / 4.4 / 15.6 / 29.9 |
| `push_v3all_act` | x75b_s2345 | EMA, corrected harness | 264 | **72.3%** | 66.7–77.4 | 57.2% | 63.3% | 87.9% | 75.4% | 78.0% | 28.3 / 19.1 / 31.0 / 55.9 | 10.5 / 5.7 / 15.4 / 24.0 |
| `push_v2_base` | seenv2 | EMA, corrected harness | 276 | **71.4%** | 65.8–76.4 | 58.3% | 62.7% | 90.6% | 72.8% | 78.3% | 23.8 / 16.3 / 22.6 / 48.6 | 10.3 / 5.2 / 14.4 / 24.7 |
| `push_v2_xattn_novec` | unseenv2 | EMA, corrected harness | 240 | **71.2%** | 65.2–76.6 | 60.4% | 62.5% | 89.6% | 73.8% | 81.7% | 23.4 / 14.2 / 27.6 / 51.6 | 9.3 / 4.7 / 11.2 / 25.0 |
| `push_v2_xattn_novec` | seenv2 | EMA, corrected harness | 276 | **69.9%** | 64.3–75.0 | 60.5% | 63.8% | 92.4% | 72.5% | 77.5% | 19.3 / 12.6 / 18.9 / 47.6 | 10.0 / 4.3 / 13.5 / 26.2 |
| `push_v2_base` | unseenv2 | EMA, corrected harness | 240 | **68.3%** | 62.2–73.9 | 54.6% | 57.5% | 86.7% | 72.9% | 79.6% | 52.0 / 16.6 / 413.1 / 57.3 | 11.2 / 5.3 / 18.3 / 29.0 |
| `push_v2_act` | seenv2 | EMA, corrected harness | 276 | **65.9%** | 60.2–71.3 | 50.0% | 57.6% | 85.5% | 71.7% | 73.6% | 29.5 / 22.4 / 27.1 / 58.8 | 10.0 / 5.3 / 12.5 / 27.1 |
| `push_v2_neither` | seenv2 | EMA, corrected harness | 276 | **65.2%** | 59.4–70.6 | 52.2% | 56.9% | 89.9% | 67.4% | 71.7% | 23.5 / 16.7 / 22.5 / 50.7 | 11.7 / 6.3 / 14.4 / 29.3 |
| `push_v3all_dp3_267m` | x75a_s3456 | EMA, corrected harness | 276 | **63.8%** | 57.9–69.2 | 42.0% | 48.2% | 78.3% | 72.8% | 70.3% | 299.4 / 26.2 / 2490.4 / 90.4 | 11.5 / 7.3 / 18.6 / 26.2 |
| `push_act_objabs` | e23 | EMA, corrected harness | 276 | **63.0%** | 57.2–68.5 | 42.0% | 49.6% | 80.4% | 71.7% | 78.6% | 31.9 / 24.9 / 27.4 / 67.1 | 9.8 / 7.4 / 10.0 / 22.0 |
| `push_act_xattn` | e23 | EMA, corrected harness | 276 | **63.0%** | 57.2–68.5 | 40.6% | 49.6% | 83.0% | 69.6% | 76.8% | 30.9 / 23.2 / 24.3 / 68.1 | 10.3 / 7.1 / 10.4 / 26.5 |
| `push_v2_act` | unseenv2 | EMA, corrected harness | 240 | **61.7%** | 55.4–67.6 | 44.6% | 49.6% | 81.2% | 66.2% | 79.6% | 32.3 / 24.3 / 30.0 / 72.2 | 11.3 / 7.8 / 15.4 / 27.1 |
| `push_v3all_dp3_267m` | x75a_s2345 | EMA, corrected harness | 276 | **56.9%** | 51.0–62.6 | 39.1% | 46.7% | 75.4% | 64.1% | 65.2% | 38.2 / 25.1 / 37.4 / 84.0 | 11.6 / 7.5 / 13.4 / 26.4 |
| `push_v3all_dp3_267m` | x75a_s1234 | EMA, corrected harness | 276 | **56.2%** | 50.3–61.9 | 38.8% | 46.7% | 77.5% | 65.9% | 64.1% | 256.2 / 27.1 / 2586.5 / 77.6 | 11.9 / 7.1 / 15.4 / 26.2 |
| `push_v2_dp3` | unseenv2 | EMA, corrected harness | 240 | **53.3%** | 47.0–59.5 | 33.3% | 41.2% | 69.6% | 67.9% | 61.7% | 46.2 / 30.5 / 43.2 / 106.1 | 10.6 / 6.3 / 11.4 / 25.4 |
| `push_dp3_objabs` | e23 | EMA, corrected harness | 276 | **51.8%** | 45.9–57.6 | 25.0% | 34.4% | 65.2% | 68.1% | 63.4% | 45.1 / 37.4 / 32.6 / 87.8 | 11.2 / 8.4 / 11.2 / 26.0 |
| `push_v2_dp3` | seenv2 | EMA, corrected harness | 276 | **51.8%** | 45.9–57.6 | 31.5% | 39.9% | 70.7% | 62.7% | 56.5% | 43.5 / 32.2 / 37.9 / 97.5 | 12.2 / 7.7 / 11.9 / 29.7 |
| `push_v3all_dp3` | x75a_s3456 | EMA, corrected harness | 276 | **49.6%** | 43.8–55.5 | 27.9% | 36.2% | 63.8% | 65.9% | 54.3% | 102.7 / 38.1 / 592.4 / 122.8 | 13.4 / 8.1 / 17.8 / 32.2 |
| `push_v3all_dp3` | x75a_s1234 | EMA, corrected harness | 276 | **48.2%** | 42.4–54.1 | 29.3% | 35.5% | 62.0% | 64.9% | 51.8% | 53.9 / 39.5 / 44.9 / 124.5 | 11.2 / 7.5 / 12.5 / 23.5 |
| `push_v3all_dp3` | x75a_s2345 | EMA, corrected harness | 276 | **47.1%** | 41.3–53.0 | 26.8% | 34.4% | 64.1% | 65.2% | 50.7% | 54.3 / 39.1 / 47.2 / 127.0 | 12.3 / 7.8 / 14.7 / 27.0 |
| `push_v3all_dp3_267m` | x75b_s2345 | EMA, corrected harness | 264 | **19.7%** | 15.3–24.9 | 10.6% | 14.8% | 37.5% | 40.5% | 25.8% | 83.7 / 72.8 / 56.7 / 162.3 | 18.2 / 13.9 / 17.4 / 35.6 |
| `push_v3all_dp3_267m` | x75b_s1234 | EMA, corrected harness | 264 | **15.9%** | 12.0–20.8 | 9.1% | 12.1% | 31.4% | 44.3% | 19.7% | 88.4 / 83.4 / 56.5 / 168.4 | 17.2 / 12.9 / 18.0 / 35.4 |
| `push_v3all_dp3_267m` | x75b_s3456 | EMA, corrected harness | 264 | **15.5%** | 11.7–20.4 | 6.1% | 8.7% | 39.0% | 37.1% | 18.6% | 127.7 / 66.9 / 742.3 / 169.9 | 19.9 / 15.0 / 18.9 / 45.0 |
| `push_v3all_dp3` | x75b_s2345 | EMA, corrected harness | 264 | **13.3%** | 9.7–17.9 | 6.4% | 8.0% | 18.6% | 66.7% | 16.3% | 116.3 / 126.9 / 60.3 / 190.9 | 10.4 / 6.1 / 11.6 / 28.7 |
| `push_v3all_dp3` | x75b_s1234 | EMA, corrected harness | 264 | **11.4%** | 8.1–15.8 | 4.9% | 7.2% | 18.2% | 67.4% | 14.0% | 111.6 / 121.5 / 54.5 / 177.2 | 9.7 / 6.9 / 10.5 / 23.6 |
| `push_v3all_dp3` | x75b_s3456 | EMA, corrected harness | 264 | **9.8%** | 6.8–14.0 | 3.8% | 6.8% | 16.3% | 62.1% | 11.7% | 116.8 / 124.0 / 54.9 / 191.2 | 12.1 / 8.0 / 12.7 / 29.3 |
| `push_dit_objdelta` | e23 | EMA | 276 | **9.1%** | 6.2–13.0 | 3.6% | 5.4% | 11.2% | 78.6% | — | 115.9 / 111.3 / 57.0 / 192.9 | 8.4 / 4.8 / 10.4 / 20.4 |

## Reading these

- Compare within an eval set and harness. Numbers before and after the settle fix are not
  comparable (the fix alone moved the baseline +2.5 pp primary / +5.4 pp strict).
- Old-stack rows (arms in cloud, per-key MIN_MAX, absolute actions) are a different regime;
  their ablation conclusions do not transfer to the new stack.
- Means and stds are dominated by the failure tail (successes ~15 ± 11 mm, failures ~75 ± 57 mm);
  medians and p90 describe typical precision and tail severity separately.
- `ever-in-goal` needs per-step traces, present only on corrected-harness evaluations.
