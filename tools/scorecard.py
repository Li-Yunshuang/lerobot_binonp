...
# ------------------------------------------------------------------------------------------
# Experiment registry: one row per evaluation, with the run's SETUP alongside every metric.
# Written to docs/push_experiments.md on every scorecard refresh.
# ------------------------------------------------------------------------------------------

SUFFIXES = {
    "k1": "K=1", "k8": "K=8", "ema": "EMA", "fixed": "corrected harness",
    "traingoals": "train-goal replay", "long": "3x horizon",
    "ns50": "50 steps", "ns100": "100 steps", "nact8": "chunk 8", "nact16": "chunk 16",
}


def parse_eval_name(name: str) -> tuple[str, list[str]]:
    """`eval_e23_ema_fixed` -> ("e23", ["EMA", "corrected harness"])."""
    parts = name.removeprefix("eval_").split("_")
    tags = []
    while len(parts) > 1 and parts[-1] in SUFFIXES:
        tags.insert(0, SUFFIXES[parts.pop()])
    return "_".join(parts), tags


def setup(run: str) -> dict:
    cks = sorted(glob(f"{RUNS}/{run}/checkpoints/*/pretrained_model/config.json"))
    if not cks:
        return {}
    cfg = json.load(open(cks[-1]))
    tp = cks[-1].replace("config.json", "train_config.json")
    data, eps, steps = "?", "?", "?"
    if os.path.exists(tp):
        ...
        data = (ds.get("repo_id") or "?").split("/")[-1]
        e = ds.get("episodes")
        eps = len(e) if isinstance(e, list) else "all"
        steps = tc.get("steps", "?")
    new_stack = str(data).startswith("push_objonly")
    return {
        "stack": "new" if new_stack else "old",
        "backbone": cfg.get("backbone", "unet"),
        "actions": cfg.get("action_space", "absolute_joint").replace("_joint", ""),
        "xattn": "yes" if cfg.get("pc_cross_attention") else "no",
        "aux": ("residual-pose" if cfg.get("num_objects", 0) else "") or
               ("future-latent" if cfg.get("future_latent_weight", 0) else "none"),
        "goal": cfg.get("goal_conditioning", "?"),
        "norm": "workspace" if cfg.get("pc_isotropic_rescale") else "min-max",
        "data": data, "episodes": eps, "steps": steps,
    }


def full_metrics(rows: list[dict]) -> dict:
    n = len(rows)
    pos = [1000 * r["pos_err_m"] for r in rows]
    ori = [DEG * r["ori_err_rad"] for r in rows]
    def rate(pt, ot):
        return 100 * sum(r["pos_err_m"] <= pt and r["ori_err_rad"] <= ot for r in rows) / n
    ever = None
    if "trace_pos_m" in rows[0]:
        def ev(r):
            p_, o_ = r["trace_pos_m"], r["trace_ori_rad"]
            return any(a <= 0.05 and b <= 0.20 for a, b in zip(p_, o_))
        ever = 100 * sum(ev(r) for r in rows) / n
    ci = wilson(round(rate(0.05, 0.20) * n / 100), n)
    return {
        "n": n, "primary": rate(0.05, 0.20), "ci": ci, "strict": rate(0.03, 0.15),
        "g40": rate(0.04, 0.15),
        "pos_gate": 100 * sum(r["pos_err_m"] <= 0.05 for r in rows) / n,
        "ori_gate": 100 * sum(r["ori_err_rad"] <= 0.20 for r in rows) / n,
        "pos": (st.mean(pos), st.median(pos), st.pstdev(pos), sorted(pos)[int(0.9 * n)]),
        "ori": (st.mean(ori), st.median(ori), st.pstdev(ori), sorted(ori)[int(0.9 * n)]),
        "ever": ever,
    }
...
def write_registry(path: str) -> None:
    rows_out = []
    for p in sorted(glob(f"{RUNS}/*/eval_*.jsonl")):
        run = p.split("/")[-2]
        eval_set, tags = parse_eval_name(os.path.splitext(os.path.basename(p))[0])
        rows = [json.loads(x) for x in open(p) if x.strip()]
        if not rows:
            continue
        m = full_metrics(keep23(rows) if eval_set in ("e75", "e23") else rows)
        rows_out.append((run, eval_set, tags, setup(run), m, len(rows)))
    ...
    for run, es, tags, su, m, _ in rows_out:
        L.append(f"| `{run}` | {es} | {', '.join(tags) or 'K=1'} | {su.get('stack','?')} | {su.get('backbone','?')} "
                 f"| {su.get('actions','?')} | {su.get('xattn','?')} | {su.get('aux','?')} | {su.get('goal','?')} "
                 f"| {su.get('norm','?')} | {su.get('data','?')} | {su.get('episodes','?')} | {su.get('steps','?')} |")
    L += [
        "",
        "## Metrics",
        "",
    ...
    for run, es, tags, su, m, _ in sorted(rows_out, key=lambda r: -r[4]["primary"]):
        pm, pmd, ps, p9 = m["pos"]; om, omd, os_, o9 = m["ori"]
        ever = f"{m['ever']:.1f}%" if m["ever"] is not None else "—"
        L.append(f"| `{run}` | {es} | {', '.join(tags) or 'K=1'} | {m['n']} | **{m['primary']:.1f}%** "
                 f"| {m['ci'][0]:.1f}–{m['ci'][1]:.1f} | {m['strict']:.1f}% | {m['g40']:.1f}% "
                 f"| {m['pos_gate']:.1f}% | {m['ori_gate']:.1f}% | {ever} "
                 f"| {pm:.1f} / {pmd:.1f} / {ps:.1f} / {p9:.1f} | {om:.1f} / {omd:.1f} / {os_:.1f} / {o9:.1f} |")
    L += [
        "",
        "## Reading these",
        "",
...
def main() -> None:
    ...
    text = "\n".join(out)
    if args.output:
        write_registry(os.path.join(os.path.dirname(args.output) or ".", "push_experiments.md"))
