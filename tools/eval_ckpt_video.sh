...
#
#   RECORD_VIDEO=6 tools/eval_ckpt_video.sh <ckpt> <ds> <repo> <list> <out.jsonl> [rollouts] [k]
set -euo pipefail
CKPT=$1; DS=$2; REPO=$3; LIST=$4; OUTF=$5; ROLL=${6:-12}; NACT=${7:-0}
REC=${RECORD_VIDEO:-6}
...
N=$(grep -c . "$LIST")
# One env per object. The 2x multiplier used elsewhere assumes ~14 objects; at 28 objects it asks
# for 56 envs, which does not fit in 24 GB (28 envs already take ~15 GB).
ENVS=$N
# n_action_steps is an inference-time slice of the predicted chunk, so it can be swept without
# retraining. 0 means "leave the checkpoint's own value alone".
NACT_ARG=""
[ "$NACT" -gt 0 ] && NACT_ARG="--num_samples $NACT"

rm -f "$SOCK"
/home/samsung/miniforge3/envs/lerobot/bin/python \
  /home/samsung/lerobot_binonp/tools/pc_policy_server.py \
  --checkpoint "$CKPT" --dataset_root "$DS" --repo_id "$REPO" --socket "$SOCK" $NACT_ARG \
...
OMNI_KIT_ACCEPT_EULA=YES /home/samsung/miniforge3/envs/isaaclab/bin/python eval_push_policy.py \
  ...
  --task push --record_video "$REC" ${EVAL_STEPS:+--steps "$EVAL_STEPS"} ${EVAL_SEED:+--seed "$EVAL_SEED"} --socket "$SOCK" --output "$OUTF" > "$LOG/eval_$TAG.log" 2>&1
